...
<template>
          ...
          <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="translate-x-full">
            <DialogPanel class="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-white py-4 pb-6 shadow-xl">
              <div class="flex items-center justify-between px-4">
                ...
                <button type="button" class="-mr-2 flex h-10 w-10 items-center justify-center rounded-md bg-white p-2 text-darkPrimary  focus:outline-none focus:ring-2 focus:ring-indigo-500" @click="open = false">
              ...
              <!-- Filters -->
              <form class="mt-4">
                <Disclosure as="div" v-for="section in filters" :key="section.name" class=" px-4 py-6" v-slot="{ open }">
                  <h3 class="-mx-2 -my-3 flow-root">
                    <DisclosureButton class="flex w-full items-center justify-between bg-white px-2 py-3 text-sm text-darkPrimary">
                      <span class="font-medium text-darkPrimary">{{ section.name }}</span>
    ...
    <div class="mx-auto max-w-3xl px-4 text-center sm:px-6 lg:max-w-7xl lg:px-8">
      ...
      <section aria-labelledby="filter-heading" class="border-t border-gray-200 py-6">
        ...
        <div class="flex items-center flex-wrap  justify-start gap-4">
          ...
          <button type="button" class="inline-block text-sm font-medium text-darkPrimary hover:text-darkPrimary sm:hidden" @click="open = true">Filters</button>
          ...
          <PopoverGroup class="hidden sm:flex sm:items-baseline sm:space-x-8">
            <Popover as="div" v-for="(section, sectionIdx) in filters" :key="section.name" :id="`desktop-menu-${sectionIdx}`" class="relative inline-block text-left">
              <div>
                <PopoverButton class="group inline-flex items-center justify-center text-sm font-medium text-darkPrimary hover:text-darkPrimary">
                  <span>{{ section.name }}</span>
                  <span v-if="sectionIdx === 0" class="ml-1.5 rounded  px-1.5 py-0.5 text-xs font-semibold tabular-nums text-darkPrimary">1</span>
                  <ChevronDownIcon class="-mr-1 ml-1 h-5 w-5 flex-shrink-0 text-darkPrimary group-hover:text-gray-500" aria-hidden="true" />
                </PopoverButton>
              </div>
                ...
                <PopoverPanel class="absolute right-0 z-10 mt-2 origin-top-right rounded-md bg-white p-4 shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none">
                  <form class="space-y-4">
                    <div v-for="(option, optionIdx) in section.options" :key="option.value" class="flex items-center">
                      ...
                      <label :for="`filter-${section.id}-${optionIdx}`" class="ml-3 whitespace-nowrap pr-6 text-sm font-medium text-darkPrimary">{{ option.label }}</label>
                    </div>
                  </form>
                </PopoverPanel>
              </transition>
          ...
          <p class="group  gap-2 inline-flex items-center justify-center text-sm font-medium text-darkPrimary hover:text-darkPrimary">
            Download
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" />
        ...
        <div class="relative mt-2 shadow-sm w-full bg-red-400 block sm:hidden ">
            <div
              class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3"
            >
              <svg
